...
function DeleteCarComponet({user , setuser}) {
  ...
  const fetchCars = async () => {
    try {
        const response = await axios.get('https://backenddeploymen.onrender.com/car/view-cars', {
            headers: {
  ...
  const handleDelete = async (carId) => {
    try {
        await axios.delete(`https://backenddeploymen.onrender.com/car/delete-car/${carId}`, {
            headers: {
              'token': JSON.parse(localStorage.getItem('token')),
            },
        });
