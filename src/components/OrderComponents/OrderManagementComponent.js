...
function DeleteCarComponet({user , setuser}) {
  ...
  const fetchorders = async () => {
    try {
        const response = await axios.get('https://backenddeploymen.onrender.com/orders/view-order', {
            headers: {
  ...
  const handleUpdation = async (ordId, value) => {
    try {
        await axios.put(`https://backenddeploymen.onrender.com/orders/confirm-order/${ordId}`,{
            status: value
        } , {
            headers: {
              'token': JSON.parse(localStorage.getItem('token')),
